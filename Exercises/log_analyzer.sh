...
echo ""
awk -F'"' '{print $2}' "$LOG_FILE" | awk '{print $1}' | sort | uniq -c | sort -rn | while read -r count method; do
    printf "  %5d  %s\n" "$count" "$method"
done
echo ""

TOTAL_BYTES=$(awk '{sum += $NF} END {print sum}' "$LOG_FILE")
if [ "$TOTAL_BYTES" -gt 1048576 ]; then
    echo "--- Total Bandwidth: $(echo "scale=2; $TOTAL_BYTES / 1048576" | bc) MB ---"
elif [ "$TOTAL_BYTES" -gt 1024 ]; then
    echo "--- Total Bandwidth: $(echo "scale=2; $TOTAL_BYTES / 1024" | bc) KB ---"
else
    echo "--- Total Bandwidth: $TOTAL_BYTES bytes ---"
fi
echo ""
...
echo "--- Requests per Hour (top 5 busiest) ---"
echo ""
sed -n 's/.*\[\([^:]*\):\([0-9]*\):.*/\2/p' "$LOG_FILE" | sort | uniq -c | sort -rn | head -5 | while read -r count hour; do
